...
Backend selection is based on live DNS ownership, not on installed programs. The `BackendKind::Resolvconf` backend is specifically Openresolv's `resolvconf(8)` implementation; it requires Openresolv 3.12 or newer, parses and verifies the Openresolv version and key store before use, and rejects passthrough mode. Classic Debian `resolvconf` is rejected rather than treated as compatible. On BSD, an exact Openresolv signature in `/etc/resolv.conf` is also required before osdns will use the backend. The key store must be identifiable through its standard state directories; custom `state_dir` layouts and complex or ambiguous `resolvconf.conf` shell syntax fail closed because osdns never sources that file.
